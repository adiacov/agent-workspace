...
(
  ...
  "$TMPBIN/bin/agent-ws" add-agent --agents custom --custom-path docs/CUSTOM_AGENT.md --no-prompt >/dev/null
  assert_file_exists docs/CUSTOM_AGENT.md

  if "$TMPBIN/bin/agent-ws" add-agent --agents custom --custom-path /tmp/bad.md --no-prompt >/tmp/custom-abs.out 2>&1; then
    fail 'absolute custom path should fail'
  fi
  assert_contains 'path must be relative' /tmp/custom-abs.out

  if "$TMPBIN/bin/agent-ws" add-agent --agents custom --custom-path ../bad.md --no-prompt >/tmp/custom-parent.out 2>&1; then
